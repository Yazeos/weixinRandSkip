 ...
 * @param  $authcode [description]
 * @return string
 * 会员密码加密或验证等
 */
function encrypt($str, $authcode=''){
    if(empty($authcode)){
        $authcode='powuMimimama';
    }
    $result = md5(md5($authcode.$str));
    return $result;
...
if ( ! function_exists('cn_substr_utf8'))
{
    function cn_substr_utf8($str, $length, $start=0){
        ...
        preg_match_all("/./su", $str, $ar);$str = '';$tstr = '';
 ...
 * md5加密字符串发送屏幕消息
 */
function sendtoserver($sendArray)
{
    $cfg = require_once(getcwd().'/socketServer/socket_conf.php');
    $host = $cfg['sendServerIP'];
    $port = $cfg['sendServerPort'];
    $t = time();$psw = md5(($t-10).'powu');
    $str = serialize($sendArray);
    $str = $str.':::'.$t.':::'.$psw;
 ...
 * 图片旋转
 */
function windExifReadData($readImg){
    $exif = @exif_read_data($readImg, 'ANY_TAG', true);
    if(!empty($exif['IFD0']['Orientation'])) {
        $image = imagecreatefromstring(file_get_contents($readImg));
