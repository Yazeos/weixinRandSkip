...
class WechatUtil extends WxCommon
{
    ...
    {                                                                               
        ...
        if (!isset($return['access_token'])) {
            $this->config['authorizer_expires'] = 0;
            Db::name('wx_user')->where('id', $wechat['id'])->update(['authorizer_expires' => 0]);
            return false;
        }

        $authorizer_expires = time() + 7000; // 提前200秒过期
        Db::name('wx_user')->where('id', $wechat['id'])->update(['authorizer_access_token'=>$return['access_token'], 'authorizer_expires'=>$authorizer_expires]);
        $this->config['authorizer_access_token'] = $return['access_token'];
        $this->config['authorizer_expires'] = $authorizer_expires;
