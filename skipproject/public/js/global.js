...
{	   	
	var upurl ='/admin.php/Uploadify/upload?num='+num+'&input='+elementid+'&path='+path+'&func='+callback+'&fileType='+fileType;
    var title = '上传图片';
    if(fileType == 'Flash'){
        title = '上传视频';
    }
...
{	   	
	var upurl ='/admin.php?m=Home&c=Uploadify&a=upload&num='+num+'&input='+elementid+'&path='+path+'&func='+callback;
    layer.open({
        type: 2,
        title: '上传图片',
        shadeClose: true,
...
function set_first_invite()
{ 
     //获取地址栏 分销推广链接id 将推荐人id 存入cookie
     var first_leader = GetUrlParams("first_invite");
     if(!(first_leader > 0)){
         first_leader = GetFirstLeaderByMode('first_invite/');
         if(first_leader == -1){
             first_leader = GetFirstLeaderByMode('first_invite=');
         } 
     }
     // 将推荐人id 存入cookie 
     if(first_leader > 0){
         setCookies('first_invite', first_leader);
     }
}
